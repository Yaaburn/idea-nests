 ...
 * 
 * Key features:
 * - Fetches metadata only (no content parsing in V1)
 * - Uses changes.list for incremental delta sync
 * - Handles trashed/deleted files
 * - Maps MIME types to artifact categories
 */
export class DriveConnector implements IConnector {
    ...
    async validateAccess(resourceId: string, _authContext: AuthContext): Promise<ValidationResult> {
        try {
            ...
            if (err.code === 403) {
                return {
                    isValid: false,
                    errorMessage: 'Access denied. Please share the folder/file with our service account.',
                };
            }
            if (err.code === 404) {
                return {
    ...
    async getResourceMetadata(resourceId: string, _authContext: AuthContext): Promise<ResourceMetadata> {
        ...
        return {
            ...
            itemCount,
            lastModified: fileInfo.data.modifiedTime ? new Date(fileInfo.data.modifiedTime) : undefined,
            additionalInfo: {
                mimeType: fileInfo.data.mimeType,
                isFolder: fileInfo.data.mimeType === 'application/vnd.google-apps.folder',
            },
        };
    }
            ...
            if (isFolder) {
                ...
                do {
                    const response = await drive.files.list({
                        q: `'${resourceId}' in parents and trashed = false`,
                        fields: 'nextPageToken, files(id,name,mimeType,size,modifiedTime,createdTime,webViewLink,thumbnailLink,lastModifyingUser)',
                        pageSize: 100,
                        pageToken,
                    });
            ...
            for (let i = 0; i < filesToProcess.length; i++) {
                ...
                try {
                    const fileData = await drive.files.get({
                        fileId,
                        fields: 'id,name,mimeType,size,modifiedTime,createdTime,webViewLink,thumbnailLink,lastModifyingUser,trashed',
                    });

                    const file = fileData.data;
                    const externalId = file.id!;
                    const lineageString = `drive:${resourceId}:file:${file.id}:${new Date().toISOString()}`;
                    ...
                    await prisma.canonicalArtifact.upsert({
                        where: { connectionId_driveFileId: { connectionId, driveFileId: file.id! } },
                        create: {
                            tenantId,
                            projectId,
                            connectionId,
                ...
                onProgress?.(10 + Math.floor((i / filesToProcess.length) * 90), 100, 'processing_files');
            }
    ...
    async deltaSync(params: DeltaSyncParams): Promise<DeltaSyncResult> {
        const { tenantId, projectId, connectionId, resourceId, cursor, onProgress } = params;
        ...
        try {
            ...
            do {
                const changesResponse = await drive.changes.list({
                    pageToken,
                    fields: 'newStartPageToken, nextPageToken, changes(fileId,file,removed,time)',
                });

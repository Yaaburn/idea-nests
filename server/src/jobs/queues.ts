...
export function createSyncWorker() {
    const worker = new Worker<SyncJobData>(
        ...
        async (job: Job<SyncJobData>) => {
            const { tenantId, projectId, connectionId, syncJobId, isInitial } = job.data;
            const idempotencyKey = `${tenantId}:${projectId}:${connectionId}:${job.id}`;

            logger.info({ jobId: job.id, connectionId, idempotencyKey }, 'Starting sync job');
            ...
            try {
                ...
                // Progress callback
                const onProgress = async (processed: number, total: number, phase: string) => {
                    await prisma.syncJob.update({
                        where: { id: syncJobId },
                ...
                if (isInitial || !connection.cursorJson) {
                    ...
                    result = await connector.deltaSync({
                        ...
                        resourceId: connection.resourceId,
                        authContext,
                        cursor: connection.cursorJson as CursorState,
                        onProgress,
                    });
                }
                ...
                await prisma.connection.update({
                    ...
                    data: {
                        ...
                        cursorJson: result.cursor ?? undefined,
    ...
    const worker = new Worker<AnalyzeJobData>(
        ...
        async (job: Job<AnalyzeJobData>) => {
            ...
            try {
                ...
                await analysisService.computeAllSignals(tenantId, projectId, signalTypes);

                logger.info({ jobId: job.id, projectId }, 'Analysis job completed');
            } catch (error) {
                logger.error({ error, jobId: job.id, projectId }, 'Analysis job failed');
                throw error;
            }
        },
        {
